...
String scriptPath = "";
String filenameParam = "filename=ESP32-CAM.jpg";
String typeParam = "&mimetype=image/jpeg";
String imageParam = "&data=";
...
HardwareSerial InterfaceSubsystem(0);
...
void setup() {
  ...
  InterfaceSubsystem.begin(9600, SERIAL_8N1, SERIAL_RX, SERIAL_TX);
  ...
  if (esp_sleep_get_wakeup_cause() == ESP_SLEEP_WAKEUP_EXT0 && digitalRead(MOTION_SENSOR) == LOW) {
    InterfaceSubsystem.println("Motion Detected");
    connectWiFi();
    delay(2000);
    configureCamera();
    delay(2000);
...
void uploadToGoogle() {
  ...
  if (!secureClient.connect(serverHost, 443)) {
    InterfaceSubsystem.println("Connection to Google Apps Script failed");
    return;
  }

  camera_fb_t *frame = esp_camera_fb_get();
  if (!frame) {
    InterfaceSubsystem.println("Camera frame error");
    return;
  }

  String encodedImage;
  char encodedChunk[base64_enc_len(3)];
  for (int i = 0; i < frame->len; i++) {
    base64_encode(encodedChunk, (char*)&frame->buf[i], 3);
    if (i % 3 == 0) encodedImage += urlEncode(encodedChunk);
  }
  esp_camera_fb_return(frame);

  String fullData = filenameParam + typeParam + imageParam;

  secureClient.println("POST " + scriptPath + " HTTP/1.1");
  secureClient.println("Host: " + String(serverHost));
  secureClient.println("Content-Length: " + String(fullData.length() + encodedImage.length()));
  secureClient.println("Content-Type: application/x-www-form-urlencoded");
  secureClient.println();

  secureClient.print(fullData);
  for (int idx = 0; idx < encodedImage.length(); idx += 1000) {
    secureClient.print(encodedImage.substring(idx, idx + 1000));
  }

  long start = millis();
  while (!secureClient.available()) {
    delay(100);
    if (millis() - start > TIMEOUT_LIMIT) {
      InterfaceSubsystem.println("Google script timeout");
      blinkLED(2);
      esp_deep_sleep_start();
      break;
    }
